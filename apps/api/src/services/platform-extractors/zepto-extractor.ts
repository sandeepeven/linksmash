 ...
 * Zepto Extractor
 *
 * This file provides a metadata extractor for Zepto product pages.
 * Parses URL structure: /pn/{product-name}/pvid/{product-variant-id}
 */
...
/**
 * Zepto product metadata extractor
 */
export class ZeptoExtractor extends BaseExtractor {
  /**
   * Checks if this extractor can handle the given URL
   *
   * @param url - The URL to check
   * @returns boolean - True if URL is a Zepto URL
   */
  canHandle(url: string): boolean {
    return isPlatform(url, "zepto");
  }

  /**
   * Extracts metadata from Zepto product page
   *
   * @param url - The Zepto URL to extract metadata from
  ...
  /**
   * Extracts metadata from Zepto URL structure
   *
   * @param url - The Zepto URL
  ...
  private extractFromUrl(url: string): ParsedMetadata {
    try {
      ...
      // Look for /pn/{product-name} pattern
      const pnIndex = pathParts.indexOf("pn");
      if (pnIndex !== -1 && pathParts[pnIndex + 1]) {
        const productName = pathParts[pnIndex + 1]
          .replace(/-/g, " ")
