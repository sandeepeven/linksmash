...
/**
 * List of hostnames that block scraping/API access
 * These sites will use default metadata without attempting to fetch
 */
...
/**
 * Checks if a URL belongs to a blocked site that prevents scraping
 *
 * @param url - The URL to check
 * @returns boolean - True if the URL is from a blocked site
 */
export function isBlockedSite(url: string): boolean {
  ...
  return BLOCKED_SITES.some((blockedSite) =>
    lowerHostname.includes(blockedSite.toLowerCase())
  );
}
